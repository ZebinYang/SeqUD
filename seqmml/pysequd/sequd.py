...
class SeqUD(object):
    ...
    def _summary(self):
        ...
        if self.verbose:
            print("SeqUD completed in %.2f seconds."%self.search_time_consumed_)
    ...
    def _generate_augment_design(self, ud_center):
        ...
        base_ud_aug = base_ud[(1+x0.shape[0]):base_ud.shape[0],:].reshape([-1, self.extend_factor_number])

        para_set_ud = np.zeros((base_ud_aug.shape[0], self.extend_factor_number))
        for i in range(self.factor_number):
            loc_min = np.sum(self.variable_number[:(i+1)])
            loc_max = np.sum(self.variable_number[:(i+2)])
            for k in range(int(loc_min), int(loc_max)):
